...
export default defineConfig({
  ...
  quiz: {
    ...
    additionalQuestions: [
      {
        question: "What is the purpose of Playwright's globalSetup in the context of authentication?",
        options: [
          "It runs all tests in a single browser instance to share login state",
          "It executes a script once before all tests to authenticate and save storageState files",
          "It automatically detects which tests need authentication",
          "It resets the database to a clean state before each test",
        ],
        correctIndex: 1,
        explanation: "globalSetup runs a single script before any test file executes. For auth, it launches a browser, logs in as each role, and saves the authenticated session (cookies + localStorage) to JSON files via storageState. Every test can then load these files to start already authenticated, eliminating repeated UI logins.",
      },
      {
        question: "Why should storageState JSON files be gitignored and never committed to version control?",
        options: [
          "They are too large and would slow down git operations",
          "They contain session tokens and credentials that are security-sensitive",
          "They are automatically regenerated by Playwright on every run",
          "They are only compatible with the OS they were generated on",
        ],
        correctIndex: 1,
        explanation: "storageState files contain session cookies, authentication tokens, and localStorage data that could grant access to the application. Committing them to version control exposes credentials. They should be regenerated fresh in globalSetup on every run and stored in a gitignored directory like ./auth/.",
      },
    ],
  },
  exercises: [
